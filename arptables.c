...
int Parse_strarray(char * str, char ** strarr)
{

	//char str[] ="arptables -P OUTPUT DROP";
//char *arr[255];
int i=0;
...
bool Arp_commands(char * p_sCmd)
{
	if(!is_ArpTableEnabled())
		return false;
	 char *cmds=malloc(strlen(p_sCmd));
	 strcpy(cmds,p_sCmd);
		...
		char *arr[255];
	    int i=Parse_strarray(cmds,arr);


		ret = do_command(i, arr, &table, &handle);
		if (ret)
...
bool Arp_OutputPolicy_Drop(bool p_bDrop)
{
	...
	char command[] = "arptables -P OUTPUT DROP";
	char commandAccept[] = "arptables -P OUTPUT ACCEPT";
	char *arr[255];
    int i=Parse_strarray(p_bDrop?command:commandAccept,arr);
...
bool is_ArpTableEnabled()
{

	char *table1 = "filter";
	char **table = &table1;
	arptc_handle_t handle1 = NULL;
	arptc_handle_t *handle = &handle1;
    bool bRet=TRUE;
