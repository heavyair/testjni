...
extern "C" {
#endif
void display_nbtstat(FILE *ofp, const struct NMB_query_response *rsp,
				int full);
...
int parse_nbtstat(const struct NMBpacket *pak, int paklen, struct NMB_query_response *, char *errbuf);
...
extern "C" {
#endif
int	sendpacket_direct( SOCKET ofd, const void *buf,
...
extern "C" {
#endif
int	recvpacket_direct( SOCKET ifd, void *buf,
...
extern "C" {
#endif
const char		*unix_errorstr(int error_no);
...
int	next_target(struct in_addr *);
...
int	target_responded(const struct in_addr *);
